# Prodesk IT - Major Internship Repository

<div align="center">
  <img src="https://img1.wsimg.com/isteam/ip/83d5de90-a5eb-47d5-a7bd-5cd38cd4c0e1/blob-8d642d8.png/:/rs=w:400,h:400,cg:true,m/cr=w:400,h:400/qt=q:95" width="150"/>
  
  ### Innovative Software Solutions Since 2012
  
  ![5 Months](https://img.shields.io/badge/Duration-14%20Jan%20to%2024%20May-blue)
  ![19 Weeks](https://img.shields.io/badge/Weeks-19%20Weeks-green)
  ![2nd Week](https://img.shields.io/badge/Status-In%20Progress-orange)
</div>

---

## 📋 Internship Overview

This repository contains all assignments, projects, and learning materials from my major internship at **Prodesk IT Company**.

- **Company**: Prodesk IT
- **Internship Type**: Major Internship
- **Start Date**: January 09, 2026
- **End Date**: May 24, 2026
- **Duration**: 19 Weeks
- **Domain**: IT & Software Development

---

## 🗂️ Repository Structure

```
prodesk-internship/
│
├── Week-01/
│   ├── assignments/
│   ├── projects/
│   └── README.md
│
├── Week-02/
│   ├── assignments/
│   ├── projects/
│   └── README.md
│
├── Week-03/
│   ├── assignments/
│   ├── projects/
│   └── README.md
│
├── ...
│
├── Week-17/
│   ├── assignments/
│   ├── projects/
│   └── README.md
│
├── Week-18/
│   ├── assignments/
│   ├── projects/
│   └── README.md
│
├── Week-19/
│   ├── assignments/
│   ├── projects/
│   └── README.md
│
├── resources/
│   ├── documentation/
│   ├── learning-materials/
│   └── references/
│
└── README.md
```

---

## 📅 Weekly Progress Tracker

| Week | Duration | Topics Covered | Status | Link |
|------|----------|----------------|--------|------|
| Week 1 | Jan 09 - Jan 16 | CSS Grid, Flexbox, Responsive Web Design, Theme Implementation | ✅ Completed | [View](https://prodeskitweek1assignment.netlify.app/) |
| Week 2 | Jan 16 - Jan 23 | - | 🔄 In Progress | [View](./Week-02) |
| Week 3 | Jan 23 - Feb 30 | - | ⏳ Upcoming | [View](./Week-03) |
| Week 4 | Jan 30 - Feb 06 | - | ⏳ Upcoming | [View](./Week-04) |
| Week 5 | Feb 6 - Feb 13| - | ⏳ Upcoming | [View](./Week-05) |
| Week 6 | Feb 13 - Feb 20 | - | ⏳ Upcoming | [View](./Week-06) |
| Week 7 | Feb 20 - Mar 27 | - | ⏳ Upcoming | [View](./Week-07) |
| Week 8 | Feb 27 - Mar 06 | - | ⏳ Upcoming | [View](./Week-08) |
| Week 9 | Mar 6 - Mar 13 | - | ⏳ Upcoming | [View](./Week-09) |
| Week 10 | Mar 13 - Mar 20 | - | ⏳ Upcoming | [View](./Week-10) |
| Week 11 | Mar 20 - Mar 27 | - | ⏳ Upcoming | [View](./Week-11) |
| Week 12 | Mar 27 - Apr 3 | - | ⏳ Upcoming | [View](./Week-12) |
| Week 13 | Apr 3 - Apr 10 | - | ⏳ Upcoming | [View](./Week-13) |
| Week 14 | Apr 10 - Apr 17 | - | ⏳ Upcoming | [View](./Week-14) |
| Week 15 | Apr 17 - Apr 24 | - | ⏳ Upcoming | [View](./Week-15) |
| Week 16 | Apr 24 - May 01 | - | ⏳ Upcoming | [View](./Week-16) |
| Week 17 | May 01 - May 08 | - | ⏳ Upcoming | [View](./Week-17) |
| Week 18 | May 08 - May 15 | - | ⏳ Upcoming | [View](./Week-18) |
| Week 19 | May 16 - May 24 | Final Project & Presentation | ⏳ Upcoming | [View](./Week-19) |

---

## 🎯 Learning Objectives

- Gain hands-on experience in software development
- Learn industry-standard development practices
- Work on real-world projects and applications
- Develop proficiency in web and mobile technologies
- Understand software development lifecycle
- Enhance problem-solving and debugging skills
- Build professional portfolio projects

---

## 🛠️ Technologies & Tools

### Frontend Development
- HTML5, CSS3, JavaScript
- React.js
- UI/UX Design Principles
- Responsive Web Design

### Backend Development
- Node.js
- RESTful APIs
- Database Management
- Server Configuration

### Tools & Platforms
- Git & GitHub
- VS Code
---

## 📚 Weekly Assignment Guidelines

Each week's folder contains:

1. **Assignments**: Weekly coding tasks and challenges
2. **Projects**: Mini projects and hands-on implementations
3. **README.md**: Detailed documentation of weekly work
4. **Code Files**: All source code with proper comments
5. **Screenshots**: Visual documentation of outputs

---

## 📊 Evaluation Criteria

- **Code Quality**: Clean, well-documented, and maintainable code
- **Problem Solving**: Effective solutions to given challenges
- **Timely Submission**: Meeting assignment deadlines
- **Innovation**: Creative approaches and implementations
- **Documentation**: Clear and comprehensive documentation
- **Collaboration**: Team work and communication skills

---

## 🚀 How to Use This Repository

1. Clone the repository:
   ```bash
   git clone https://github.com/rohitsingh/prodesk-IT-internship.git
   ```

2. Navigate to specific week:
   ```bash
   cd prodesk-IT-internship/Week-01
   ```

3. Review assignments and projects in each week's folder

4. Check individual README files for detailed instructions

---

## 📞 Contact Information

**Company**: Prodesk IT  
**Email**: infoprodesk@gmail.in  
**Phone**: +123456789  
**Office Hours**: Monday - Saturday (10:00 AM - 7:00 PM)

**Intern Details**:  
- **Name**: Rohit Singh Gouria
- **Email**: rohitsingh139915@gmail.com
- **LinkedIn**: [Connect on LinkedIn](https://www.linkedin.com/in/rohit-singh-gouria-475923278/)
- **GitHub**: [github.com/rohitsingh](https://github.com/Rohit551800)

---

## 📝 Updates & Changelog

This repository is updated regularly with new assignments and projects.

### Recent Updates
- **Jan 22, 2026**: Week 1 completed - Grid, Flexbox, Responsive Design, Themes
- **Jan 14, 2026**: Repository initialized, Internship commenced

---

## 🏆 Achievements & Milestones

- ✅ Week 1: Mastered CSS Grid and Flexbox layouts
- ✅ Week 1: Built responsive webpages with media queries
- ✅ Week 1: Implemented dark/light theme switching
- ⏳ Mid-term evaluation
- ⏳ Final project completion

---
## 🤝 Acknowledgments

Special thanks to:
- **Dr. Amit Maheshwari** - Founder, Prodesk IT
- **Mentors and Supervisors** at Prodesk IT
- **Fellow Interns** for collaboration and support

---

## 📄 License

This repository is for educational purposes as part of the internship program at Prodesk IT.

---

<div align="center">
  
  ### 💡 "Transforming Ideas Into Digital Reality"
  
  **© 2026 Prodesk IT. All rights reserved.**
  
</div>
